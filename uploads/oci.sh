...
Available_Domain='Kxst:AP-SEOUL-1-AD-1'
# 镜像                   [source_id]
Image_ID='ocid1.image.xxxx'
# 子网ID                 [subnet_id]
Subnet_ID='ocid1.subnet.xxxx'
# 公钥                   [ssh_authorized_keys]
SSH_Key_PUB="ssh-rsa xxxx"
# 租户ID                 [compartment_id]
...
_Instance_ID="ocid1.instance.oc1.ap-seoul-1.xxxx"
# 升级到CPU个数
_CPU=4
# 升级到内存大小(GB)
_RAM=24
...
launch_instance() {
    msg_text="开始刷鸡「${Instance_Name}: ${CPU}C${RAM}G」"
    info "$msg_text"
    ...
    while [ true ]; do
        ...
        409)
            ...
            ;;
        *)
            if [ -n "$(echo "$ret" | grep -i "error")" ]; then
            ...
            text_success="牛牪犇逼,实例「${Instance_Name}: ${CPU}C${RAM}G」抢到了,正在创建."
            info "${text_success}"
...
update_instance() {
    msg_text="开始升鸡加配到「${_CPU} Core CPU, ${_RAM} GB RAM」"
    info "$msg_text"
    sendMessage "$msg_text"
    while [ true ]; do
        _warn "正在尝试升级实例..."
        ...
        *)
            ...
            text_success="牛牪犇逼,实例已加配到「${_CPU} Core CPU, ${_RAM} GB RAM」."
            info "${text_success}"
            sendMessage "${text_success}"
            echo "$ret" > ./success.json 2>&1
            sleep 3s
...
sendMessage() {
    if [ 1 -eq $SEND_MSG ]; then
        if [ 1 -eq $PROXY ]; then
            result=$(curl --connect-timeout 10 --max-time 10 -s -S -x $PROXY_URL -X POST $URL -d parse_mode=Markdown -d chat_id=${CHAT_ID} -d text="*${TTNAME}*%0A${1}" 2>&1)
            if [ 0 -eq $? ]; then
            ...
            result=$(curl --connect-timeout 10 --max-time 10 -s -S -X POST $URL -d parse_mode=Markdown -d chat_id=${CHAT_ID} -d text="*${TTNAME}信息*%0A${1}" 2>&1)
            if [ 0 -eq $? ]; then
                info "Telegram 消息提醒发送成功"
            else    
                error "Telegram 消息提醒发送失败, $result"
